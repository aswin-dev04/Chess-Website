...
class Game {
  ...
  public:
    Game(Board& board) : board(board), playerTurn(true), whiteInCheck(false), blackInCheck(false), checkMate(false), draw(false) {};
    ...
    inline void setWhiteInCheck(bool whiteInCheck) { this->whiteInCheck = whiteInCheck; }

    inline bool isBlackInCheck() const { return blackInCheck; }
    inline void setBlackInCheck(bool blackInCheck) { this->blackInCheck = blackInCheck; }
